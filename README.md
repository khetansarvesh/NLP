...
> [!IMPORTANT]
> It has been proved previously that its better to first finetune LLM on any task using SFT (called mid training) and then finetune on the same task using RL (called post training) followed by human alignment training using RL (called post training), it gives better outcomes. This [notebook](https://colab.research.google.com/github/unslothai/notebooks/blob/main/nb/Qwen3_(4B)-GRPO.ipynb) from UnSloth follows the below recepie to convert Qwen3 from a non reasoning model to a reasoning model.
