...
- We will learn finetuning a LLM using RL using [<ins>Reasoning to Generate Positive Sentiments Task</ins>](). Here we finetune an LLM to do reasoning and then generate positive sentiments for a given input non reasoning. It has been proved previously that its better to first finetune LLM on reasoning dataset using SFT and then finetune it on the resoning dataset using RL.
